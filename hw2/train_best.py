...
train_x = pd.read_csv(sys.argv[1], encoding = 'Big5')#.values.astype(np.float)
train_y = pd.read_csv(sys.argv[2], encoding = 'Big5').values.astype(np.float)
test_x = pd.read_csv(sys.argv[3], encoding = 'Big5')#.values.astype(np.float)
...
def onehotencoding(train_x):
	...
	train_x = pd.concat([train_x, df2, df3, df4, df5, df6, df7, df8], axis = 1).values.astype(np.float)
...
def augmentation(temp):
	mylist = [1, 2, 3, 4, 5, 6]
	mylist1 = [7, 8, 9, 10, 11, 12]
	for i in mylist1:
		square = np.multiply(temp[:,i], temp[:,i])
		square = square.reshape(-1, 1)
		temp = np.hstack((temp, square))
	return temp
...
train_x = augmentation(train_x)
test_x = augmentation(test_x)
...
w, bias = logistic_regression(32, 30000, w, bias, 0.9)
#print(w, bias)
correct = 0.0
